...
DIM S11  AS INTEGER
DIM S16  AS INTEGER
DIM 머리회전각도 AS INTEGER
DIM 머리회전방향 AS INTEGER
...
DIR G6A,1,0,0,1,0,0		'모터0~5번 왼다리
DIR G6D,0,1,1,0,1,1		'모터18~23번 오른다리
DIR G6B,1,1,1,1,1,1		'모터6~11번 왼팔
DIR G6C,0,0,0,1,1,0		'모터12~17번 오른팔
...
RX_EXIT5:
	IF KEYSTATE = 5 THEN
		ETX 4800, 5
		GOTO MAIN
	ELSEIF KEYSTATE = 37 THEN
		ETX 4800, 195
		ERX 4800, A, RX_EXIT
		'KEYSTATE = 38
	ELSEIF KEYSTATE = 31 THEN
		ETX 4800, 31
	ENDIF
	
	IF A = 200 THEN
		KEYSTATE = 38
		ERX 4800, A, Head_Trans
		'GOSUB MAIN777
		'KEYSTATE
	ELSE
		GOTO MAIN
...
MOVE G6A, 100,  76, 145,  93, 100, 100
MOVE G6D, 100,  76, 145,  93, 100, 100
MOVE G6B, 100,  30,  80, 100, 100, 169
MOVE G6C, 100,  30,  80, 190, 100, 100
WAIT
...
'    ETX 4800, KEYSTATE ' 동작 멈춤 확인 송신 값
    
'MAIN26: '라벨설정

'    ETX 4800, 26 ' 동작 멈춤 확인 송신 값
    
'MAIN37: '라벨설정

'    ETX 4800, S11 ' 동작 멈춤 확인 송신
...
'     ETX 4800, 5 ' 동작 멈춤 확인 송신 값
MAIN: '라벨설정

    ETX 4800, 230 ' 동작 멈춤 확인 송신 값
...
MAIN_2:
    ...
    ON A GOTO MAIN,KEY1,KEY2,KEY3,KEY4,KEY5,KEY6,KEY7,KEY8,KEY9,KEY10,KEY11,KEY12,KEY13,KEY14,KEY15,KEY16,KEY17,KEY18 ,KEY19,KEY20,KEY21,KEY22,KEY23,KEY24,KEY25,KEY26,KEY27,KEY28 ,KEY29,KEY30,KEY31,KEY32, KEY33, KEY34, KEY35, KEY36, KEY37
